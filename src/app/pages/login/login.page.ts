...
export class LoginPage implements OnInit, OnDestroy {
  ...
  checkValidEmail(event: any) {
    if (this.signupForm.controls['email'].status === 'VALID') {
      this.apiService.validateEmail(event.target.value)
        .then((resp: any) => {
          if (resp.code === StatusCodes.PK_EMAIL_EXISTS) {
            this.commonService.showAlert(AlertTypeEnum.Error, '', 'This email is already registered.', this.commonService.alertButtonList);
          } else if (resp.code !== StatusCodes.PK_SUCCESS) {
            this.commonService.showAlert(AlertTypeEnum.Error, '', 'Something went wrong! Try again.', this.commonService.alertButtonList);
          }
        }).catch((error: any) => {
          this.commonService.showAlert(AlertTypeEnum.Error, '', error.message || 'Failed to validate email. Please try again.', this.commonService.alertButtonList);
        });
    }
  }
  ...
  verifyCode() {
    ...
    if (!ref_code) return;
    this.organization = '';
    this.apiService.verifyReferralCode(ref_code, email)
      .then((resp: any) => {
        if (resp.code === StatusCodes.PK_SUCCESS) {
          ...
          this.sf['codeVerified'].setValue(false);
          this.commonService.showAlert(AlertTypeEnum.Error, '', resp.message || 'Invalid referral code.', this.commonService.alertButtonList);
        }
      })
      .catch(() => {
        this.sf['codeVerified'].setValue(false);
        this.commonService.showAlert(AlertTypeEnum.Error, '', 'Failed to verify code. Please try again.', this.commonService.alertButtonList);
      });
  }
      ...
      .then(async (res) => {
        if (res.code === StatusCodes.PK_SUCCESS) {
          ...
          if (loginResponse && loginResponse.token) {
            ...
            if (decodedToken && decodedToken.end_date && new Date(decodedToken.end_date) < new Date()) {
              this.commonService.showAlert(AlertTypeEnum.Warning, '', 'Your Organization subscription has expired. Please contact your administrator.', this.commonService.alertButtonList);
              return;
            }
        ...
        else if (res.code === StatusCodes.PK_INPUT_INCORRECT || res.code === StatusCodes.PK_INVALID_CREDENTIALS) {
          this.commonService.showAlert(AlertTypeEnum.Error, '', res.message || 'Incorrect email or password. Please try again.', this.commonService.alertButtonList);
        }
        else {
          this.commonService.showAlert(AlertTypeEnum.Error, '', res.message, this.commonService.alertButtonList);
          return;
        }
      })
      .catch((err: any) => {
        this.commonService.showAlert(AlertTypeEnum.Error, '', err.message || 'Please try again.', this.commonService.alertButtonList);
      });
  }
  ...
  onSignup() {
    if (this.signupForm.invalid) {
      ...
      if (!this.sf['codeVerified'].value) {
        this.commonService.showAlert(AlertTypeEnum.Warning, '', 'Please verify your referral code first.', this.commonService.alertButtonList);
      }
      return;
    }
    const { name, email, exp, institution, ref_code, password } = this.signupForm.value;
    const payload = {
      name, email, exp, password, institution, ref_code,
      daily_goal: '', googleUser: false,
    };
    this.commonService.showLoader();
    this.apiService.signup(payload)
      .then((resp: any) => {
        if (resp.code === StatusCodes.PK_SUCCESS) {
        this.commonService.showAlert(AlertTypeEnum.Success, '', 'Account Created Successfully');
        ...
        setTimeout(() => {
          ...
          this.commonService.showAlert(AlertTypeEnum.Error, '', resp.message || 'Signup Failed');
        }
      })
      .catch((err: any) => {
        console.error('Signup error:', err);
        this.commonService.showAlert(AlertTypeEnum.Error, '', err.message || 'Please try again.');
      });
  }
